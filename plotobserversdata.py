...
def step1_get_data(year, month, observer_id=None, region_id=None, get_new=True, make_pickle=False, data_description=None):
    ...
    if data_description is not None:
        ...
        if region_id is not None:
            pickle_file_name = '{0}plotregiondata_{1}_{2}{3:02d}.pickle'.format(env.local_storage, region_id, year, month)
        else:
            if observer_id is not None:
                pickle_file_name = '{0}plotobserversdata_{1}_{2}{3:02d}.pickle'.format(env.local_storage, observer_id, year, month)
            else:
                ml.log_and_print("[warning] plotoserversdata.py -> step1_det_data: Need ObserverID and/or ForecastRegionTID to make this work.", print_it=True)
                return []
    ...
    if get_new:
        all_observations = go.get_all_registrations(from_date, to_date, output='DataFrame', geohazard_tids=10,
        ...
        for d in _get_dates(from_date, to_date, dt.timedelta(days=1)):
            ...
            for i in all_observations.index:
                ...
                if this_date == d:
                    ...
                    # list of all regids - this counts occurrences
    ...
    if data_description is not None:
        ...
        if region_name is not None:
            ...
            if observer_name is not None:
                ...
                ml.log_and_print("[warning] plotobserversdata.py -> step2_plot: Need ObserverID and/or forecastRegionTID to make this work.", print_it=True)
    ...
    if data_description is not None:
        ...
        if region_name is not None:
            ...
            if observer_id is not None:
                ...
                ml.log_and_print("[warning] plotobserversdata.py -> step3_make_html: Got to have region and/or observer to make this work.", print_it=True)
...
def make_observer_plots(observer_list, months):
    ...
    for k,v in observer_list.items():
        for m in months:
...
def make_svv_plots(run_all=False):
    ...
    observer_list = [79, 43, 1084, 33, 119, 67, 101, 41, 34, 125, 126, 8, 384, 955, 14, 841, 50, 1123,
                     199, 1598, 1646, 637, 1664, 135, 307, 1279, 1203, 3393, 1312, 3436, 3496]

    region_ids = [3007, 3009, 3010, 3011, 3012, 3013, 3014, 3015, 3016, 3017, 3022, 3023, 3024, 3027, 3028, 3029,
                  3031, 3032, 3034, 3035]
...
def make_2017_18_plots(run_all=False):
    ...
    :param run_all: [bool] If True only last month is plotted. If False, whole season is plotted
    :return:
    """
    ...
    # if not specified run only the last month (and the one before that if we are just at the beginning of a month).
    if not run_all:
    ...
    observer_list = gm.get_observer_dict_for_2017_18_plotting(limit_for_select=5)
...
def make_2015_16_plots(run_all=False):
    ...
    # Get all regions
    region_ids = gm.get_forecast_regions('2015-16')
...
def test_sort_observer_list():
    ...
    observer_list_unsorted = [[k,v] for k,v in observer_dict.items()]
...
if __name__ == "__main__":
    ...
    months = []
    month = dt.date(2017,12,1)
    while month < dt.date.today():
        months.append(month)
        almost_next = month + dt.timedelta(days=35)
        month = dt.date(almost_next.year, almost_next.month, 1)

    # months = [dt.date(2017,11,1)]
    # make_observer_plots(observer_list, months)
    make_region_plots([3017], months)
